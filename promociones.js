...
// ── Traslado de mercadería entre sedes (Fase 6 — la data ya soporta esto desde stockPorSede) ──
function abrirModalTraslado() {
  // Corregido: usaba la sede de login, no la sede efectiva del selector de admin — un admin
  // operando en Sede II via el selector veia el traslado armado sobre Sede I sin darse cuenta.
  const origen = sedeAdminEfectiva();
  const sel = document.getElementById('trsl-prod');
  sel.innerHTML = DB.productos.map(p => `<option value="${p.id}">${p.nombre} (${stockEnSede(p, origen)} ${p.unidad} en ${origen})</option>`).join('');
  document.getElementById('trsl-origen').value = origen;
  const _sedesConocidas = ['principal', 'Tienda Aleze II'];
  const otrasSedes = _sedesConocidas.filter(s => s !== origen);
  document.getElementById('trsl-destino').innerHTML = otrasSedes.map(s => `<option value="${s}">${s}</option>`).join('');
  document.getElementById('trsl-cantidad').value = '';
  _traspasoActualizarInfo();
  abrirModal('modal-traslado');
}

function _traspasoActualizarInfo() {
  const prodId = parseInt(document.getElementById('trsl-prod').value);
  const prod = DB.productos.find(p => p.id === prodId);
  const origen = sedeAdminEfectiva();
  const info = document.getElementById('trsl-stock-info');
  if (prod && info) info.textContent = `Disponible en ${origen}: ${stockEnSede(prod, origen)} ${prod.unidad}`;
}
...
async function confirmarTraslado() {
  ...
  const origen = sedeAdminEfectiva();
  const destino = document.getElementById('trsl-destino').value;
  if (!destino) { alert('Selecciona la sede de destino.'); return; }
  const cantidad = parseFloat(document.getElementById('trsl-cantidad').value);
  if (!cantidad || cantidad <= 0) { alert('Ingresa una cantidad válida.'); return; }
  const disponible = stockEnSede(prod, origen);
  if (cantidad > disponible) { alert(`No hay suficiente stock en ${origen}. Disponible: ${disponible} ${prod.unidad}`); return; }
  if (!confirm(`¿Confirmar traslado?\n\n${prod.nombre}: ${cantidad} ${prod.unidad}\nDesde: ${origen}\nHacia: ${destino}`)) return;

  // Paquete atomico: el descuento en origen y el aumento en destino viajan juntos. Sin esto,
  // si solo uno de los dos llegara a guardarse, el stock quedaria "perdido en el camino" —
  // descontado de una sede sin haber llegado nunca a la otra, exactamente el riesgo real.
  ...
  // Ambos ajustes van en UNA sola escritura al mismo documento (stock/{id}) — llamar set()
  // dos veces sobre el mismo documento dentro de un lote no garantiza que ambos cambios
  // se combinen, es mas seguro mandarlos juntos en un solo objeto.
  batch.set(docM(dbModular, 'stock', String(prod.id)), {
    [`stockPorSede.${origen}`]: incrementM(-cantidad),
    [`stockPorSede.${destino}`]: incrementM(cantidad)
  }, { merge: true });
  const _movId = getId();
  const _movData = { id:_movId, tipo:'traslado', desc:`Traslado: ${prod.nombre} x${cantidad} ${prod.unidad} — ${origen} → ${destino}`, monto:0, hora:nowTime(), fecha:today(), usuario:currentUser, sedeId: origen };
  batch.set(docM(dbModular, 'movimientos', String(_movId)), _movData);

  _sincIniciar('traslado_lote', prod.id);
  try {
    await batch.commit();
    _sincTerminar('traslado_lote', prod.id);
  } catch (e) {
    _sincError('traslado_lote', prod.id, e, 'el traslado — no se aplicó nada, el stock sigue completo en la sede de origen');
    return;
  }
  if (!DB.movimientos) DB.movimientos = [];
  DB.movimientos.push(_movData);

  // NUNCA usar ajustarStockSede() acá — esa función escribe a Firestore por su cuenta
  // (fbIncrementarStock), lo que aplicaría el mismo cambio DOS VECES ya que el lote de arriba
  // ya lo escribió. Se actualiza el estado local a mano, igual que en el resto de las
  // funciones con lote atómico (venta, fiado, merma, canje, boleta).
  if (!prod.stockPorSede) prod.stockPorSede = { principal: prod.stock || 0 };
  prod.stockPorSede[origen]  = Math.max(0, Math.round(((prod.stockPorSede[origen]||0)  - cantidad) * 1000) / 1000);
  prod.stockPorSede[destino] = Math.max(0, Math.round(((prod.stockPorSede[destino]||0) + cantidad) * 1000) / 1000);
  prod.stock = stockTotal(prod);
  fbGuardarProductos();
  cerrarModal('modal-traslado');
  renderInvTable();
  alert(`✅ Traslado registrado: ${cantidad} ${prod.unidad} de ${origen} a ${destino}.`);
}
